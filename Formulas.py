...
def TaxasEquivalentes():
    ...
    print("""
        ...
        (1+taxa Efectiva)^numero de subperiodos no tempo = (1+taxa anual)""")
    juroAnual = input("    Taxa de juros anual (decimal): ")
    numeroSub = input("    Numero de subperiodos no ano (semestral = 2): ")
...
def VaAnuidadeSemCrescimento(): 
    ...
    valueDict[nameToSave] = solution
    myList = ["anuidade", "taxaDeAtualizacao", "periodoAtual", "taxaDeAtualizacao", "periodoAtual", "taxaDeAtualizacao", "valorAtual"]
    printList(myList)
...
def FatorAnuidade():
    ...
    """
    print("--Fator Anuidade 3--")
    print("Antes de introduzir o valor da taxa de atualizacao atualiza-la")
    taxaDeAtualizacao = input("   Taxa de atualização: ")
    periodoAtual = input("    Periodo de atualização: ")
...
def MapaDeCashFlow():
    ...
    inputFile.write(str(taxaImposto)+ "\n")
    taxaAtualizacao = float(input("   Taxa de Atualização (decimal): "))
    inputFile.write(str(taxaAtualizacao)+ "\n")
    ...
    Aux[0][0] =  Matrix[0][0] = " Rubrica/Periodo--------------------------------------------------"
    Aux[1][0] =  Matrix[1][0] = " (-) Despesas de Investimento-------------------------------------"
    Aux[2][0] =  Matrix[2][0] = " (+) Valor Residual do Investimento-------------------------------" 
    Aux[3][0] =  Matrix[3][0] = " CashFLow do Investimento(=1+2)-----------------------------------"
    Aux[4][0] =  Matrix[4][0] = " Vendas-----------------------------------------------------------"
    Aux[5][0] =  Matrix[5][0] = " Custos fixos - Gastos Pessoal------------------------------------"
    Aux[6][0] =  Matrix[6][0] = " Custos fixos - Manutenção, Seguro e Energia----------------------"
    Aux[7][0] =  Matrix[7][0] = " Custos Variavéis - Fabrico---------------------------------------"
    Aux[8][0] =  Matrix[8][0] = " Custos Variáveis - Administração---------------------------------"
    Aux[9][0] =  Matrix[9][0] = " Depreciações + Amortizações--------------------------------------"
    Aux[10][0] = Matrix[10][0] =  " Resultado Operacional(EBIT)--------------------------------------"
    Aux[11][0] = Matrix[11][0] =  " Resultado Líquido[EBIT x (1-t)]----------------------------------"
    Aux[12][0] = Matrix[12][0] =  " CFExploração = (Depreciações + Amortizações) + Resultado Líquido-"
    Aux[13][0] = Matrix[13][0] =  " CFTotal  = CFInvestimento + CFExploração-------------------------"
    Aux[14][0] = Matrix[14][0] =  " CFAtualizado  = CFTotal * Fator Atualização[1/(1+r)**n]----------"
    Aux[15][0] = Matrix[15][0] =  " CFAcumulado------------------------------------------------------"
    ...
    for numColuna in range(1, numerodeperiodos+1): 
        ...
        if(numColuna >= 2):
            ...
            inputFile.write(str(amortizacoes)+"\n")
            if(numItensDesval >= 1):
        ...
        inputFile.write(str(Matrix[1][numColuna])+ "\n")
        ...
        if(numColuna == numerodeperiodos):
            for x in range(numItensDesval):
                ...
                inputFile.write(str(valorMercado)+ "\n")
                if(x == 0):
        ...
        if(numColuna == numerodeperiodos):            
            ...
            Aux[3][numColuna] = f"{Matrix[1][numColuna]}+{Matrix[2][numColuna]} = {Matrix[3][numColuna]}"
            
        else:
            Matrix[3][numColuna] = Matrix[1][numColuna]
            Aux[3][numColuna] = f"{Matrix[1][numColuna]} = {Matrix[3][numColuna]}"


        # Vendas
        valVendas = int(input(f"    Vendas do Periodo {numColuna-1}: "))
        inputFile.write(str(valVendas)+ "\n")
        ...
        if(numColuna == 2):
            ...
            inputFile.write(str(Matrix[5][numColuna])+ "\n")
            Aux[5][numColuna] = f"{valCustosFixPess}"
        
        elif(numColuna > 2 and (int(input("  Custos fixos - Gastos Pessoal estão iguais? (0 ou 1): ")) == True)):    
            Matrix[5][numColuna] = valCustosFixPess
            inputFile.write("1"+ "\n")
            Aux[5][numColuna] = f"{valCustosFixPess}"
        ...
        if(numColuna == 2):
            ...
            inputFile.write(str(Matrix[6][numColuna])+ "\n")
            Aux[6][numColuna] = f"{valCustosFixMan}"

        elif(numColuna > 2 and (int(input("  Custos fixos - Manutenção, Seguro e Energia estão iguais? (0 ou 1): ")) == True)): 
            Matrix[6][numColuna] = valCustosFixMan
            inputFile.write("1"+ "\n")
            Aux[6][numColuna] = f"{valCustosFixMan}"
        ...
        if(numColuna >= 2):
            ...
            inputFile.write(str(valCustosVarFab)+ "\n")
        ...
        if(numColuna >= 2):
            ...
            inputFile.write(str(valCustosVarAdmin)+ "\n")
        ...
        Aux[10][numColuna] = f"{Aux[4][numColuna]} {Aux[5][numColuna]} {Aux[6][numColuna]} {Aux[7][numColuna]} {Aux[8][numColuna]}+{Matrix[9][numColuna]}={Matrix[10][numColuna]}"

        # Resultado Líquido 
        aplicavel = int(input("    imposto é aplicável? (1 ou 0): "))
        inputFile.write(str(aplicavel)+ "\n")
        if(Matrix[10][numColuna] < 0):    
            if(aplicavel == True):
                Matrix[11][numColuna] = Matrix[10][numColuna] * (1 - taxaImposto)
                Aux[11][numColuna] = f"{Matrix[10][numColuna]}(= /\)*(1 - {taxaImposto}) = {Matrix[11][numColuna]}"
        ...
        Aux[14][numColuna] = f"{Matrix[13][numColuna]}/((1+{taxaAtualizacao})**({numColuna} - 1))=" + "{:.2f}".format(Matrix[14][numColuna])

        # Cashflow Acumulado
        if(numColuna != 1):
            Matrix[15][numColuna] = Matrix[15][numColuna-1] + Matrix[14][numColuna]
            Aux[15][numColuna] = "{:.2f}".format(Matrix[15][numColuna-1]) + " + " + "{:.2f}".format(Matrix[14][numColuna]) + "=" + "{:.2f}".format(Matrix[15][numColuna])
    ...
    with open(f"mapas/CashflowMap-{current_time}.txt", 'w') as f: 
        for x in range(16):
            for y in range(0, numerodeperiodos+1):
                if(y == 0):
                    print("| " + Aux[x][y], end=" ", file=f)
                else:
                    print(" | ", end="", file=f)
    ...
    for x in range(16):
        for y in range(0, numerodeperiodos+1):
            if(y == 0):
                espacoColuna[y] = len(str(Matrix[x][y]))  
            elif(espacoColuna[y] < len(str(Matrix[x][y]))):
                espacoColuna[y] = len(str(Matrix[x][y]))
    ...
    for x in range(16):
        for y in range(0, numerodeperiodos+1):
            if(y == 0):
                ...
                print(" | ", end="")
                for i in range(0, espacoColuna[y]-len(str(Matrix[x][y]))):
                    print(" ", end="")
                print(str(Matrix[x][y]) + " | ", end=" ")
        print("\n", end="")
    print("\n")
